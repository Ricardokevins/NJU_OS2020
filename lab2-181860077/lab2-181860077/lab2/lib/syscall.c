...
void printf(const char *format,...){
	...
	while(format[i]!=0)
	{
		if(format[i]=='%')
		{
			switch(format[++i])
			{
				case 'd': decimal=*((int*)paraList);paraList+=4;count=dec2Str(decimal,buffer,MAX_BUFFER_SIZE,count);break;
				case 's': string =*((char**)paraList);paraList+=4;count=str2Str(string,buffer,MAX_BUFFER_SIZE,count);break;
				case 'c': character=*((char*)paraList);paraList+=4;buffer[count]=character;count++;break;
				case 'x': hexadecimal=*((uint32_t*)paraList);paraList+=4;count=hex2Str(hexadecimal,buffer,MAX_BUFFER_SIZE,count);break;
			}
		}
		else
		{
